#!/bin/bash
# AI-CMS文章迁移脚本 v3
# 在AI-CMS服务器(192.168.31.120)上直接运行，自动下载附件
...
# 1. 获取公司CMS文章列表（无需登录，公开API）
echo "1. 获取CMS文章列表..." >> $LOG_FILE
POSTS=$(curl -s "$API_CMS/wp-json/wp/v2/posts?per_page=100" 2>/dev/null)
...
for ARTICLE in $CMS_ARTICLES; do
    ...
    # 获取文章详情
    POST_DATA=$(curl -s "$API_CMS/wp-json/wp/v2/posts/$POST_ID" 2>/dev/null)
    ...
    echo "下载了 $IMG_COUNT 张图片" >> $LOG_FILE
    
    # 替换内容中的图片链接
    NEW_CONTENT=$(echo "$CONTENT" | sed "s|http://a591977220.synology.me:8888/wp-content/uploads|$API_CMS/web/images|g")
...
" 2>/dev/null)
    ...
    if [ "$VERIFY_TITLE" = "$TITLE" ]; then
        echo "✅ 验证通过: $TITLE (含$IMG_COUNT张图片)" >> $LOG_FILE
        MIGRATED=$((MIGRATED + 1))
        REPORT_CONTENT="$REPORT_CONTENT\n- ✅ $TITLE (含$IMG_COUNT张图片)"
    else
        echo "❌ 验证失败: $TITLE" >> $LOG_FILE
        FAILED=$((FAILED + 1))
        REPORT_CONTENT="$REPORT_CONTENT\n- ❌ $TITLE (验证失败)"
    fi
    ...
    # 每迁移1篇后等待确认
    echo ""
    echo "====== 已迁移 $((MIGRATED+SKIPPED+FAILED)) 篇，暂停等待确认 ======"
    echo "迁移: $MIGRATED, 跳过: $SKIPPED, 失败: $FAILED"
...
echo "- 跳过（已存在）: $SKIPPED 篇" >> $REPORT_FILE
echo "- 迁移失败: $FAILED 篇" >> $REPORT_FILE
echo "" >> $REPORT_FILE
